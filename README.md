...
영상 검토의 **추적 공백 (검토 전)** 목록을 선택하면 해당 구간의 시작 프레임으로
이동합니다. **이전 공백 / 다음 공백**은 현재 위치 앞뒤의 공백 시작점으로 이동하며,
같은 시작점의 여러 트랙은 한 번에 지나갑니다. 좌우 방향키 또는 타임라인 옆 버튼으로
한 프레임씩 이동할 수 있습니다. 시간은 영상 시작 기준이며 표시 프레임 번호는 1부터
시작합니다. 구간을 방문하는 것만으로 경고가 해제되지는 않습니다.

모든 항목이 문제없이 가려져야 **완료**로 끝납니다. 실패, 건너뜀, 검출 영역
없는 저장이 하나라도 있으면 **검토 필요**와 요약을 표시합니다. 작업이 끝나면
활동 영역의 **파일별 결과…**에서 확인 필요·실패 항목을 필터링하고, 상세 원인과
입력·출력 폴더를 확인할 수 있습니다. 결과는 다음 작업을 시작할 때 초기화되며,
처리를 시작하지 않은 파일은 목록에 표시하지 않습니다.
