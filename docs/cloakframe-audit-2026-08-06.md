...
재확인 revision: `9edfa0ce2713e2dd09edf8ddd0e545cf6de7ed06` (`main`). 아래 줄
...
| CF-019 | Fixed | 저장 경로가 `cloakframe_core`의 `saveModelFile`(`ModelStore.cpp`)로 옮겨졌고, `ModelDownloader.cpp`는 그것을 호출만 한다. 임시 파일은 이름이 무작위이고 배타적으로(POSIX `O_CREAT|O_EXCL|O_NOFOLLOW`, Windows `CREATE_NEW` + `FILE_FLAG_OPEN_REPARSE_POINT`) 만들어지며, 쓴 뒤 같은 descriptor로 되읽어 SHA-256을 다시 확인하고, 그 다음에야 한 번의 replace로 모델 이름을 넘겨받는다. POSIX는 `rename`, Windows는 검증한 handle을 닫지 않고 `FileRenameInfoEx`의 replace/POSIX semantics를 사용하므로 동시에 게시하는 다른 writer와 경합해도 검증한 파일의 identity를 잃지 않는다. 다른 writer가 같은 digest를 먼저 게시한 경우도 성공으로 수렴한다. 따라서 이미 있던 모델은 교체 순간까지 그대로 남고 실패하면 손대지 않은 채로 남는다. `inspectCacheDirectory`로 폴더가 다른 계정 소유이거나 group/other 쓰기 가능이면 저장 자체를 거부한다. `tests/test_model_store.cpp`가 게시·잔여물 없음·예측 가능한 이름 선점·symlink 선점·실패 시 기존 모델 보존·digest 불일치·동시 다운로드·경합 중 동일 digest 수렴·공개 폴더 거부를 검사하며, 이 중 셋은 수정 전 동작에서 실패함을 확인했다 |
...
2026-08-10에 update signing key 두 개를 만들어 등록하면서 이 목록의 두 번째
항목은 닫혔다 — macOS의 Sparkle EdDSA key (`CF-018`)와 Windows/Linux의 Ed25519
key (`CF-001`). 두 private key 모두 이 저장소가 아니라 만든 사람의 keychain과
오프라인 사본에 있다. 어느 쪽이든 잃어버리면 그 key로 서명된 build를 받은
client가 이후 모든 update를 영구히 거부하므로, 기계를 옮길 때 함께 옮겨야 한다.

`CF-001`의 서명 경로는 local round trip과 CI build까지 확인했지만, 실제
release에서 동작하는 것은 다음 `v*` tag가 처음이다. 그때 release job 로그에
`signed <package> <digest>`가 찍히고 release asset에 `.nupkg.sig`가 올라오는지
확인한다.
